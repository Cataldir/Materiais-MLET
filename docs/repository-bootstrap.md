...
The repository is not scoped to a single cohort. MLET6, MLET7, MLET8, and MLET9 can all appear as provenance labels for sessions, but documentation should lead with reusable curriculum structure instead of a local cohort status narrative.

## Branch Model

| Branch | Role |
| --- | --- |
| `canonica` | Editorial baseline for curated reusable material. |
| `main` | Public aggregation branch for approved material and approved overlays. |

## Cross-Repository Boundary

| Repository | Responsibility | Examples |
| --- | --- | --- |
| `mlet` | Private governance, intake, provenance, scheduling, professor follow-up, escalation, and operational decisions. | Live schedule, issue triage, content authoring status, email evidence, internal assignments. |
...
5. When coverage differs by cohort, document the gap as public material coverage rather than as professor-management status.
...
4. Keep cohort pages focused on public material readiness, not internal professor follow-up.
